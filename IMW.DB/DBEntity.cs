...
/// </summary>
    ...
    /// </summary>
        public void PrepareQueries(object instance, string tableName, string endingPropertyName, ref string insertQueryStart, List<PropertyInfo> properties)
        {
            ...
            insertQueryStart = Convert.ToString("INSERT INTO [") + tableName + "] (";
            foreach (PropertyInfo property in properties)
            {
                name = GetFieldName(property.Name);
                insertQueryStart += Convert.ToString("[") + name + "],";
            ...
            if (entity.ExtraFields is object)
            {
                foreach (string field in entity.ExtraFields)
                    insertQueryStart += Convert.ToString("[") + GetFieldName(field) + "],";
            }
    ...
    /// TODO: Update summary.
    /// </summary>
        public void PrepareQueries(object instance, string tableName, string endingPropertyName, ref string insertQueryStart, List<DocumentFields> properties)
        {
            DBEntity entity = (DBEntity)instance;
            string name = string.Empty;
            insertQueryStart = Convert.ToString("INSERT INTO [") + tableName + "] (";
    ...
    /// </summary>
        public string PrepareInsertQuery(object instance, string insertQueryStart, string endingPropertyName, List<PropertyInfo> properties)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                l_Query += l_Param + Convert.ToString(",");
    ...
    /// <returns>Update Query</returns>
        public string PrepareUpdateQuery(object instance, string tableName, string primaryKey, string endingPropertyName, List<PropertyInfo> properties)
        {
            ...
            foreach (PropertyInfo l_Property in properties)
            {
                ...
                if (string.IsNullOrEmpty(l_Criteria) && (l_Name.CompareTo(primaryKey) == 0 || ("Temp" + l_Name).CompareTo(primaryKey) == 0))
                {
                    if (("Temp" + l_Name).CompareTo(primaryKey) == 0)
                    {
                        ...
                        l_Query += Convert.ToString(Convert.ToString("[") + l_Name + "] = ") + l_Param + ",";
                        l_Name = "Temp" + l_Name;
                        property = type.GetProperty(l_Name);
                        l_Param = string.Empty;
                        l_Type = GetFieldType(property, instance);
                        ...
                        l_Criteria = Convert.ToString(Convert.ToString(" WHERE [") + primaryKey + "] = ") + l_Param;
                    }
                    else
                    {
                        l_Criteria = Convert.ToString(Convert.ToString(" WHERE [") + primaryKey + "] = ") + l_Param;
            ...
            l_Query = l_Query.Substring(0, l_Query.Length - 1);
            l_Query = Convert.ToString(Convert.ToString(Convert.ToString("UPDATE [") + tableName + "] SET ") + l_Query) + l_Criteria;
            return l_Query;
        }
    ...
    /// <returns>Update Query</returns>
        ...
        public string PrepareUpdateQueryNonPrimaryKey(object instance, string tableName, string endingPropertyName, List<PropertyInfo> properties)
        {
            ...
            l_Query = l_Query.Substring(0, l_Query.Length - 1);
            l_Query = Convert.ToString(Convert.ToString(Convert.ToString("UPDATE [") + tableName + "] SET ") + l_Query);
            return l_Query;
        }
            ...
            if (entity.ExtraFields is object)
            {
                ...
                foreach (string field in entity.ExtraFields)
                {
                    ...
                    if (primaryKeys.Contains(l_Name))
                    {
                        if (string.IsNullOrEmpty(l_Criteria))
                        {
                            l_Criteria = Convert.ToString(Convert.ToString(" WHERE [") + l_Name + "] = ") + l_Param;
                        }
                        else
                        {
                            l_Criteria += Convert.ToString(Convert.ToString(" AND [") + l_Name + "] = ") + l_Param;
                        }
                    }
                    else
                    {
    ...
    /// <returns>Query to Delete a Record From DB.</returns>
        public string PrepareDeleteQuery(object instance, string tableName, string primaryKey)
        {
            ...
            return Convert.ToString(Convert.ToString("DELETE FROM [") + tableName + "]") + l_Criteria;
        }
    ...
    /// <returns>Query to Get the Object</returns>
        public string PrepareGetObjectQuery(object instance, string tableName, string primaryKey)
        {
            ...
            return Convert.ToString(Convert.ToString("SELECT * FROM [") + tableName + "]") + l_Criteria;
        }
        ...
        private void PopulateProperty(object instance, string name, PropertyInfo property, DataRow row)
        {
            ...
            switch (type)
            {
                case Declarations.FieldTypes.Boolean:
                    {
                        property.SetValue(instance, Conversions.ToBoolean(PublicFunctions.ConvertNull(row[name], false)), null);
                        break;
                    }
        ...
        private string PrepareCriteria(object instance, List<string> primaryKeys)
        {
            ...
            if (property is null)
            {
                foreach (string key in primaryKeys)
                {
                    ...
                    if (string.IsNullOrEmpty(l_Criteria))
                    {
                        l_Criteria = Convert.ToString(Convert.ToString(" WHERE [") + key + "] = ") + l_Param;
                    }
                    else
                    {
                        l_Criteria += Convert.ToString(Convert.ToString(" AND [") + key + "] = ") + l_Param;
                    }
                }
            }
            else
